...
const Slide = ({ activeIndex, data }) => {
  ...
  if (data) {
    content = data.slice(activeIndex, activeIndex + 4).map((s, index) => {
      ...
      for (let categoryObj of state.categoryList) {
        category = Object.values(categoryObj)[0].find(
          (subcategoryObj) => Object.keys(subcategoryObj)[0] === subcategory
        );
        if (category) {
      ...
      return (
        <Link
          key={index}
          to={s.product_detail ? `/${category}/${subcategory}/${product}` : "#"}
        >
          <div className={styles.Item}>
            ...
            <p className={styles.Text}>
              ...
              <span>{s.name || s.product}</span>
            </p>
            <p className={styles.Text}>
              <Price className={styles.Icon} />
              <span>{commafy(s.price)} تومان</span>
            </p>
          </div>
        </Link>
      );
  ...
  return <section className={styles.Slide}>{content}</section>;
};
